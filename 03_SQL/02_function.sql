...
select concat('$',format(salary, 2)) as salary # $xxxxx.xx 형태로 show
from   emp;
...
select emp_id,
       ...
       salary,
       ceil(salary * 1.15) as "SAL_RAISE",
       ceil(salary * 1.15) - salary as diff
from   emp;
...
select curdate(); -- date
select curtime(); -- time
-- insert into member(id, ....., join(date) values (?, ?, ..., curdate())
...
select hire_date,
       ...
       adddate(hire_date, interval 10 week) "입사 10개월 후"
from   emp
where  dept_name = 'IT';
